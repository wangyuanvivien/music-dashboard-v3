...
# --- 2. Data Cleaning and Preparation (Robust for New Data) ---
@st.cache_data
def prepare_data(df):
    ...
    # Ensure all count/score columns are numeric/Int64 for safe math/sorting
...
def show_dashboard(df_filtered):
    ...
    # ... (Rest of show_dashboard remains the same, using df_filtered)
    ...
    # TOP TRACKS TABLES SECTION (Uses the updated display_cols with year)
...
# --- 7. Time Series Dashboard Function (REFINED) ---
def show_time_series_dashboard(df):
    ...
    trend_data = df_ts.groupby('release_year')[metrics_cols].mean().reset_index()
    ...
    # Create two columns for view/like count and popularity
    ...
    fig_feature = px.line(
        trend_data,
        x='release_year',
        y=['danceability', 'timbre'],
        title='Average Audio Features (Danceability & Timbre) Over Time',
    ...
    if THEME_COL in df_ts.columns:
        # Count the themes per year
        theme_counts = df_ts.groupby(['release_year', THEME_COL]).size().reset_index(name='Count')
        
        # Create a stacked bar chart showing composition changes
        fig_theme = px.bar(
            theme_counts,
            x='release_year',
            y='Count',
